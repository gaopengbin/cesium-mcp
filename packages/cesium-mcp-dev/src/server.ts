...
const DEV_VERSION = typeof __VERSION__ === 'string' ? __VERSION__ : '0.0.0-dev'

export function buildDevMcpServer(): McpServer {
  ...
  server.registerTool(
    'cesium_api_lookup',
    {
      description: '查询 Cesium API 文档。输入类名或关键词，返回对应的 API 描述、构造参数、常用方法和示例代码。',
  ...
  server.registerTool(
    'cesium_code_gen',
    {
      description: '根据自然语言描述生成 Cesium 代码片段。支持常见场景：视图控制、图层加载、Entity 创建、样式设置等。',
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      description: '交互式构建 Cesium Entity 配置。选择 Entity 类型（point/billboard/label/polyline/polygon/model），返回完整的 Entity JSON 配置和创建代码。',
      inputSchema: z.object({
