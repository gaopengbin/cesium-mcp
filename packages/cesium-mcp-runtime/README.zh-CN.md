...
工具按 **12 个工具集** 组织。默认启用 4 个核心工具集（30 个工具）。可通过环境变量或由 AI 智能体在运行时动态激活额外的工具集。共享标题、行为标注、多语言描述、默认值、输入校验、对外发布的输出 Schema 和结构化结果统一从 `cesium-mcp-contracts` 的标准 JSON Schema 生成；旧客户端仍可读取文本 `content`。
...
|----------------------|------|
| *（未设置）* | 默认 4 个工具集（30 工具 + 2 元工具） |
| `view,entity,camera,animation` | 仅启用指定的工具集 + 2 元工具 |
| `all` | 全部 62 个命令工具，无元工具 |
