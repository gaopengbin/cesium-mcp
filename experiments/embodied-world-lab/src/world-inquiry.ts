...
export class LabWorldInquiry {
  ...
  observe(at: string, actor: SpatialCoordinate, goal: SpatialCoordinate, hazard?: SpatialCoordinate): void {
    ...
    const objects = [
      { id: 'actor', name: t('角色'), position: actor, source: 'live-pose', basis: t('当前执行器位姿') },
      { id: 'goal', name: t('观察点'), position: goal, source: 'task-metadata', basis: t('已授权的任务目标坐标') },
      ...(hazard ? [{ id: 'hazard', name: t('落石区'), position: hazard, source: 'fixture', basis: t('测试障碍已触发发现；不是视觉识别') }] : []),
    ]
    const observation: WorldObservation = {
      ...
      limitations: [t('仅包含当前角色、授权目标及已发现的测试障碍，不是完整场景扫描。')],
  ...
  accepts(command: string): boolean {
    return /^(查看场景|场景有什么|记录现场|比较变化|有什么变化|查找\s*.+|寻找\s*.+|inspect scene|record scene|compare changes|find\s+.+)[？?。！!]*$/i.test(command)
  }

  answer(command: string, at: string): string {
    if (/^(记录现场|record scene)/i.test(command)) {
      this.baseline = `checkpoint-${this.sequence}`
      this.memory.checkpoint(this.baseline, at)
      return t('已记录本次可查询证据。之后输入“比较变化”，比较两次记录；未观测区域不作结论。')
    }
    if (/^(比较变化|有什么变化|compare changes)/i.test(command)) {
      if (!this.baseline) return t('请先输入“记录现场”，建立明确的比较基线。')
      return formatResult(this.memory.execute({ kind: 'changes', checkpointId: this.baseline, at }))
    }
    const target = command.match(/^(?:查找|寻找|find)\s*(.+?)[？?。！!]*$/i)?.[1]
...
function formatResult(result: WorldMemoryResult): string {
  const lines = [t('场景证据查询（本地结构化查询，未调用视觉模型）：')]
  if (result.kind === 'changes') {
    lines.push(...result.changes.map(change => `${change.after.object.name ?? change.objectId}：${
      change.kind === 'newly-observed' ? t('本次新记录到，不代表刚刚生成') : t('观测属性变化（{0}）', change.fields.join('、'))
    }`))
    if (!result.changes.length) lines.push(t('两次记录中没有检测到对象属性变化；不能据此断言整个场景没有变化。'))
  } else {
    lines.push(...result.objects.map(item => {
      ...
      return `${item.object.name ?? item.objectId}：${position?.map(value => value.toFixed(5)).join(', ') ?? t('位置未知')}${item.freshness === 'stale' ? t('（证据已过期）') : ''}`
    }))
    if (!result.objects.length) lines.push(t('现有证据中没有匹配目标，不能确认它不存在。'))
  }
  for (const citation of result.citations) {
    lines.push(t('证据 {0}/{1}：{2}，{3}', citation.reference.observationId, citation.reference.evidenceId, citation.basis, citation.reference.sampledAt))
  }
  return lines.join('\n')
}
