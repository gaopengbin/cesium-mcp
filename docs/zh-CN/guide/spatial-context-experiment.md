...
地图工具逐条显示出来；飞行射线发现前方障碍后，也会单独触发一次模型绕行决策，
随后由本地净空约束决定接受还是安全接管。当前模型读取的是结构化射线测量，尚未
读取第一视角图像，因此页面不会再把它描述成视觉模型感知。
...
这里演示的是有边界的世界感知，不是“全知”。系统会明确报告探测范围与命中证据，
不会声称发现尚未加载的几何，也不会把任意像素的语义理解当成已经解决的问题。
...
- 独立视角、5 条有限射线、动态障碍探测、局部重规划与实际轨迹分离展示组成的
  闭环飞行实验。
...
5. ✅ 增加由独立视角、Cesium 公共射线相交、滚动重规划和可见执行证据组成的
   Grounded Flight Loop。
6. 只有真实场景需要跨场景记忆时，再考虑持久化 World Graph。
